...
class AnimSimpleDemoActivity : Activity(), IAnimListener {
    ...
    private val dir by lazy {
        // 存放在sdcard应用缓存文件中
        externalCacheDir?.absolutePath ?: Environment.getExternalStorageDirectory().path
    }

    // 视频信息
    data class VideoInfo(val fileName: String, val md5: String)

    private val fileList = listOf<VideoInfo>(
        VideoInfo("video_4k_h265_13000_30f.mp4", "31e0ac16e35a222d753ed8590f1bfa0a"),
        VideoInfo("video_h265_14000_30f.mp4", "29052a5970a3bd7eb7b0203332d3c8fd")
    )
    ...
    private fun initTestView() {
        findViewById<LinearLayout>(R.id.btnLayout).visibility = View.VISIBLE
        /**
         * 开始播放按钮
         */
        ...
        findViewById<Button>(R.id.seekTo1).setOnClickListener {
            animView.seekTo(0, 15000)
        }
        findViewById<Button>(R.id.seekTo2).setOnClickListener {
            animView.seekTo(1, 0)
        }
    }

    private fun loadFile() {
        val files = fileList.map { it.fileName }.toTypedArray()
