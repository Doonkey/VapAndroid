...
class HardDecoder(player: AnimPlayer) : Decoder(player), SurfaceTexture.OnFrameAvailableListener {
    ...
    private var isSeekReq = false
    private var seekToWindow: Pair<Int, Long> = Pair(0, 0)
    ...
    override fun seekTo(windowSeekTo: Pair<Int, Long>) {
        isSeekReq = true
        seekToWindow = windowSeekTo
    }
        ...
        renderThread.handler?.post {
            try {
                glTexture?.apply {
                    ...
                    render?.setUpdateMatrix(transformMatrix)
                    render?.renderFrame()
    ...
    private fun startDecode(extractor: MediaExtractor, decoder: MediaCodec, durationMs: Long) {
        ...
        while (!outputDone) {
            ...
            if (isSeekReq) {
                val targetWindow = seekToWindow.first
                val targetTime = seekToWindow.second
                if (targetWindow != player.playlistIndex) {
                    if (targetWindow >= 0 && targetWindow < player.playlist.size) {
                        player.playlistIndex = targetWindow
                        isSwitching = true
                        recycleDecoder(decoder, extractor)
                        renderThread.handler?.post {
                            player.startPlay(player.playlist[targetWindow])
                        }
                        return
                    } else {
                        isSeekReq = false
            ...
            if (!outputDone) {
                ...
                when {
                    ...
                    decoderStatus < 0 -> {
                        ...
                        onVideoProgress(bufferInfo.presentationTimeUs / 1000, durationMs)
    ...
    private fun recycleDecoder(decoder: MediaCodec?, extractor: MediaExtractor?) {
        decoder?.apply {
            stop()
            release()
        }
