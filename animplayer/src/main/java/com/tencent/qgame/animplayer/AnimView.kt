...
open class AnimView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    override fun seekTo(window: Int, millisecond: Long) {
        player.windowSeekTo = Pair(window, millisecond * 1000)
    }
    ...
    override fun startPlay(fileContainer: IFileContainer) {
        ...
        player.addMediaItem(fileContainer)
        startPlayInner(fileContainer)
    }

    override fun addMediaItem(file: File) {
        addMediaItem(FileContainer(file))
    }

    override fun addMediaItem(assetManager: AssetManager, assetsPath: String) {
        addMediaItem(AssetsFileContainer(assetManager, assetsPath))
    }

    override fun addMediaItem(fileContainer: IFileContainer) {
        player.addMediaItem(fileContainer)
    }
    ...
    override fun startPlay() {
        ...
        startPlayInner(player.playlist[0])
    }


    private fun startPlayInner(fileContainer: IFileContainer) {
        ui {
